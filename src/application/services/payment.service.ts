...
class PaymentService {
  ...
  async getPaymentsStatus(args: any) {
    ...
    const { loanId, status, payment_date, clientId,userId } = filter;
  ...
  async getPayments(args: any) {
    ...
    const { loanId, status, payment_date, clientId,client,userId } = filter;
...
if (clientIds.length) {
  query.client = { $in: clientIds };
    ...
    console.log(query, " queryyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy");
  ...
  static async generatePayments(loan: any) {
    const { installment_number, amount, _id, payment_interval,user_id } = loan;
    ...
    for (let index = 0; index < Number(installment_number); index++) {
      ...
      console.log("Generating payments for loan:", nuevoPago);
    }
