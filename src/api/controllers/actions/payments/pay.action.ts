...
class PayAction {
  ...
  async request(req: Request, res: Response, next: any) {
    try {
      ...
      console.log(amount,"paidddddddddddddddddddddddddddddddd amount")
  ...
  async setLoanValues(paidAmount: number) {
    ...
    this.loan.progress  = Math.floor((this.loan.paid_amount/this.loan.total_amount) *100)
  ...
  async setClientValues(paidAmount: number) {
    ...
    console.log(payment_date)
        ...
        const onDate = today.toISOString().split("T")[0] == payment_date.toISOString().split("T")[0] ? 
        true :
    ...
    } else if (this.payment.status == "incomplete") {
      this.client.statics.payments.pending -= 1; 
